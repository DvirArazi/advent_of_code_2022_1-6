...
fn priority(item: char) -> i32 {
    item as i32 +
    if item.is_uppercase() { -('A' as i32) + 26 }
    else { -('a' as i32) }
    + 1
}

fn main() {
    let packs: Vec<&str> = DATA.split('\n').collect();

    let mut tri_packs = vec![vec![""; 3]; packs.len()/3];
    for i in 0..packs.len() {
        tri_packs[i/3][i%3] = packs[i];
    }

    let mut score = 0;
    for tri_pack in tri_packs {
        println!("{:?}", tri_pack);
        for item in tri_pack[0].chars() {
            if tri_pack[1].contains(item) && tri_pack[2].contains(item) {
                score += priority(item);
